...
def main():
    ...
    copy_files(config["input_path"], config["output_path"], config["new_filename_format"])

    print(bcolors.GREEN + "\u2714 Files correctly copied to your output directory" + bcolors.ENDC)
    print()

    files = os.listdir(config["output_path"])
    printProgressBar(0, len(files), prefix = 'Modifying metadata:', suffix = 'Complete', length = 50)

    for i,file in enumerate(files):
        printProgressBar(i + 1, len(files), prefix = 'Modifying metadata:', suffix = 'Complete', length = 50)
...
def copy_files(from_path: str, to_path: str, filename_format:str):
    ...
    printProgressBar(0, len(files), prefix = 'Copying files:', suffix = 'Complete', length = 50)

    for i,file in enumerate(files):
        printProgressBar(i + 1, len(files), prefix = 'Copying files:', suffix = 'Complete', length = 50)
        ...
        if file_extension == ".mp3":
            ...
            new_file_name = get_new_filename(filename_format, file, os.path.join(os.getcwd(), from_path, file))
    ...
    for key in ["artist", "title", "album", "discnumber", "tracknumber"]:
        if ("{" + key + "}") in filename_format:
            if EasyID3(path).get(key) and len(EasyID3(path).get(key)[0]) > 0:
                name_to_return = name_to_return.replace("{" + key + "}", EasyID3(path).get(key)[0].split("/")[0])
            else:
